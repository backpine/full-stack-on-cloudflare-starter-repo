...
export class LinkClickTracker extends DurableObject {
    ...
    async alarm() {
		console.log('alarm')
        const clickData = getRecentClicks(this.sql, this.mostRecentOffsetTime); // get clicks since last sent to clients
        
        const sockets = this.ctx.getWebSockets();
        for (const socket of sockets) {
			socket.send(JSON.stringify(clickData.clicks)); // send new clicks to client
		} // end for sockets

        await this.flushOffsetTimes(clickData.mostRecentTime, clickData.oldestTime); // update offset times in storage
        await deleteClicksBefore(this.sql, clickData.oldestTime); // delete clicks older than oldest sent to clients
